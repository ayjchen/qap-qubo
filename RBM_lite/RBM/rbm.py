...
class RBM():
    ...
    """
    Sample hidden units from visible_probabilities (or activations)
    Uses flip-the-state operator for the transition operator
    visible_probabilities - torch.Tensor containing visible probabilities (or activations)
    previous_hidden - Current hidden activations at time of function call
    """
    def sample_hidden_flip(self, visible_probabilities, previous_hidden, temperature=1):
        hidden_activations = torch.matmul(visible_probabilities, self.weights/temperature) + self.hidden_bias/temperature
        ones = torch.ones_like(hidden_activations)
        zeros = torch.zeros_like(hidden_activations)
        hidden_probabilities = torch.where(previous_hidden == 0, torch.min(torch.exp(hidden_activations), ones), torch.max(1 - torch.exp(-hidden_activations), zeros))
        return torch.where(hidden_activations == 0, 0.5*ones, hidden_probabilities)

    """
    Sample visible units from hidden_probabilities (or activations)
    Uses flip-the-state operator for the transition operator
    hidden_probabilities - torch.Tensor containing hidden probabilities (or activations)
    previous_visible - Current visible activations at time of function call
    """
    def sample_visible_flip(self, hidden_probabilities, previous_visible, temperature=1):
        visible_activations = torch.matmul(hidden_probabilities, self.weights.t()/temperature) + self.visible_bias/temperature
        ones = torch.ones_like(visible_activations)
        zeros = torch.zeros_like(visible_activations)
        visible_probabilities = torch.where(previous_visible == 0, torch.min(torch.exp(visible_activations), ones), torch.max(1 - torch.exp(-visible_activations), zeros))
        return torch.where(visible_activations == 0, 0.5*ones, visible_probabilities)
    ...
    def tensgenerate_sample(self, num_samps, k=1, input_visible=None, input_hidden = None, clamp=None, use_outbits=True, use_zeros=True, temperature=1, flip = False):
        if use_outbits:
            if not clamp is None:
                temp = clamp
                clamp = torch.zeros(self.num_visible, device=torch.device(self.device)) - 1
        ...
        if use_outbits:
            return visible_activations[:, self.outbits]
        if flip:
            return visible_activations, hidden_activations
        return visible_activations


    def tensgenerate_statistics(self, num_samples, num_chains, k=1, clamp=None,
                                reset_cycles=-1, use_outbits=True, use_zeros=True, input_data=None,
        ...
        if input_data is None:
            ...
            sample = input_data
        # if keep_samps:
        #     samps = torch.zeros(num_samples, num_chains, bitmap.size(0))
        # else:
        #     samps = []
        samps = []
